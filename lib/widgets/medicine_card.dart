...
class MedicineCard extends StatefulWidget {
  ...
  @override
  State<MedicineCard> createState() =>
      _MedicineCardState();
}

class _MedicineCardState
    extends State<MedicineCard> {
  Timer? _timer;
  ...
  void _calcularTiemposRestantes() {
    ...
    for (int i = 0; i < med.horas.length; i++) {
      try {
        if (med.pospuestas[i] &&
            med.postponedUntil[i].isNotEmpty) {

          final partes =
          med.postponedUntil[i].split(':');
          ...
          final diff =
              hasta.difference(ahora).inSeconds;
            ...
            Wrap(
              ...
              children: List.generate(
                med.horas.length,
                  ...
                  bool isSnoozed =
                      med.pospuestas[index] &&
                          !isTaken;

                  int secsLeft =
                      _secondsRemaining[index] ?? 0;
